...
for (modelA, modelB), group in groups:
    ...
    x_labels = [layer.get_text() if len(layer.get_text().split("_")) > 1 
               and (int(layer.get_text().split("_")[1])+1)%2 == 0
               else "" 
               for layer in  ax[0].get_xticklabels()]
    y_labels = [layer.get_text() if len(layer.get_text().split("_")) > 1 
               and (int(layer.get_text().split("_")[1])+1)%2 == 0
               else "" 
...
def plot_loss_sequence(run_id, *, stitch_mean= None, tune_mean=None, stitch_error=None, tune_error=None, color=None, label=None, bin_steps=10, ax=None):
    ax = ax or plt.gca()
...
# Calculate the simple moving average
    ...
    Y_tu_avg = np.concatenate((np.array([list(hist_stitch["value"])[-1]]), 
                              np.convolve(Y_finetune, weights, mode='valid')))
    ...
    if stitch_mean != None and stitch_error != None:
        x_point = 0
        y_point = [stitch_mean]
        lognormal_quantiles_errorbars(x_point, y_point, yerr=[stitch_error], input_type="linear", color=color, ecolor=color, capsize=3, label=(label.split(" ")[-1] + " stitching validation mean"))
    
    if tune_mean != None and tune_error != None:
        x_point = [list(hist_finetune["step"])[-1] + 25]
        y_point = [tune_mean]
        lognormal_quantiles_errorbars(x_point, y_point, yerr=[tune_error], input_type="linear", color=color, ecolor=color, capsize=3, label=(label.split(" ")[-1] + " fine-tuning validation mean"))
...
with open("analysis/summary_table.txt", "w") as file:
    ...
    file.write("\\hline\n")
    file.write("Best Stitching & Downstream & Stitching Loss & Fine-tuning Loss & Stitching Accuracy & Fine-tuning Accuracy & loss($A$) $>$ loss($B$) & loss($A$) $<$ loss($B$) \\\\\n")
...
for downstream, group in df.groupby("downstream"):
    ...
    for key, grp in df.groupby("downstream"):
        ...
        if is_rank_order_violation:
            ...
            mvdf = df[ (df["metric_name"] == "loss") & (df["phase"] == "downstream")].set_index('run_id')
            ...
            mvdf_stitch = df[ (df["metric_name"] == "loss") & (df["phase"] == "stitching")].set_index('run_id')
            mvdf_stitch = df[ (df["metric_name"] == "loss") & (df["phase"] == "stitching")].set_index('run_id')
            ...
            print(key, winner1["upstream"], winner1["metrics.downstream-modelAxB-val-acc1"], winner2["upstream"], winner2["metrics.downstream-modelAxB-val-acc1"])
